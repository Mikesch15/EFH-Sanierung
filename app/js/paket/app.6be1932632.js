...
function formular6(Z2, a, art) {
  ...
  modalOeffnen({
    titel: (entwurf5.id ? "Bearbeiten: " : "Neu: ") + (entwurf5.art === "pendenz" ? "Pendenz / Mangel" : "Arbeit"),
    koerper: koerper5(Z2),
    speichern: () => speichern5(Z2)
  });
}
...
async function speichern5(Z2) {
  ...
  try {
    if (dateiWartend4) {
      const info = await hochladen(dateiWartend4, Z2.projektId, "arbeiten");
      daten.datei_pfad = info.datei_pfad;
      daten.datei_name = info.datei_name;
    }
...
function aktion8(a, knopf, Z2) {
  if (a === "arbeit-neu") return formular6(Z2, null, "arbeit");
  if (a === "pendenz-neu") return formular6(Z2, null, "pendenz");
  if (a === "arbeit-bearbeiten") {
    const eintrag = (Z2.arbeiten || []).find((x) => x.id === knopf.dataset.id);
    return formular6(Z2, eintrag, eintrag ? eintrag.art : "arbeit");
  }
  if (a === "arbeit-stand") {
    const eintrag = (Z2.arbeiten || []).find((x) => x.id === knopf.dataset.id);
    if (!eintrag) return;
